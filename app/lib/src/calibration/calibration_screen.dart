...
class CalibrationDialog extends StatefulWidget {
  const CalibrationDialog({
    ...
    this.initialMinCutoff = 1.0,
    this.initialBeta = 0.05,
    this.onFilterParamsChanged,
    super.key,
  });

  final GestureInputSource source;

  /// Starting slider values — pass the last-persisted settings so reopening
  /// calibration doesn't silently reset smoothing back to factory defaults.
  final double initialMinCutoff;
  final double initialBeta;

  /// Called on every slider change, in addition to applying it to [source]
  /// directly — lets the caller persist the value without polling.
  final void Function(double minCutoff, double beta)? onFilterParamsChanged;
...
class _CalibrationDialogState extends State<CalibrationDialog> {
  ...
  late double _minCutoff = widget.initialMinCutoff;
  late double _beta = widget.initialBeta;
        ...
        children: [
          ...
          _FilterSlider(
            ...
            onChanged: (v) {
              ...
              widget.onFilterParamsChanged?.call(v, _beta);
            },
          ),
          ...
          _FilterSlider(
            ...
            onChanged: (v) {
              ...
              widget.onFilterParamsChanged?.call(_minCutoff, v);
            },
          ),
        ],
      );
